...
class TopSalesBlockList extends ItemsList
{
	...
	function selectDbList()
    {
		...
		{
			$query[] = "(SELECT * FROM catalog WHERE top > 0 AND visible = 1 AND arhiv = 0 LIMIT {$value}, 1)";
		}
	...
	function selectDbCount()
    {
		...
		$query = "SELECT COUNT(*) FROM catalog WHERE top > 0 AND visible = 1 AND arhiv = 0;";
